...
import { getAreas } from 'actions/routes';
  ...
  render() {
    return (
      ...
      <div className="app-wrapper">
        <ContainerHeader match={this.props.match} title="Manage Stockist"/>
        ...
        <MaterialTable
          ...
          columns={[
            ...
            {
              title: 'Area', field: 'area_id',
              render: (rowData) => {
                return rowData.area.name
              },
              editComponent: props => (
                <FormControl className="w-100 mb-2">
                  <InputLabel htmlFor="age-simple">Area</InputLabel>
                  <Select
                    value={props.value}
                    onChange={e => props.onChange(e.target.value)}
                    input={<Input id="area_id"/>}
                  >
                    <MenuItem value="">
                      <em>None</em>
                    </MenuItem>
                    {this.props.areas_data.map((area) => {
                      return (
          ...
          data={query =>
            new Promise((resolve, reject) => {
              let url = 'v1/stockists?'
                url += '&page=' + (query.page + 1)
                axios.get(url,
                ).then(({data}) => {
                  console.log("fetchStockist: ", data);
                  if (data.success) {
          ...
          editable={{
            ...
            onRowUpdate: (newData, oldData) =>
                new Promise((resolve, reject) => {
                  ...
                  delete newData.structure_id
                    setTimeout(() => {
                      axios.put('v1/stockists/'+newData.id,
                        newData
                        ).then(({data}) => {
                          console.log("post: ", data);
                          if (data.success) {
            ...
            onRowDelete: oldData =>
                new Promise((resolve, reject) => {
                    setTimeout(() => {
                      axios.delete('v1/stockists/'+oldData.id
                        ).then(({data}) => {
                          console.log("post: ", data);
                          resolve();
                        }).catch(function (error) {
