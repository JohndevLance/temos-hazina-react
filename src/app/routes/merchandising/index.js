...
class Merchandising extends React.Component {
  notify(type, msg){
    if(type=="success"){
  ...
  createNewForm = () => {
    ...
    this.props.resetForm(() => {
      this.props.history.push({pathname: "/app/formbuilder", props: this.props});
    });
  };
  export = (form_id) => {
    this.setState({
    ...
    setTimeout(() => {
      ...
      let url = 'v1/surveyreport/'+form_id
  ...
  render() {
    return (
      ...
      <div className="app-wrapper">
        <ContainerHeader match={this.props.match} title="Manage Merchandising"/>
        <Button
          ...
          columns={[
            ...
            { title: 'identifier', field: 'identifier' },
            {
              ...
              render: (rowData) => {
                ...
                return (
                  <CloudDownloadRounded onClick={() => { this.export(rowData.id) }}></CloudDownloadRounded>
                  )
              }
            },
          ]}
          ...
          editable={{
            ...
            onRowUpdate: (newData, oldData) =>
                new Promise((resolve, reject) => {
                  console.log(newData)
                    setTimeout(() => {
                      axios.patch('v1/channels/'+newData.id,
                        newData
                        ).then(({data}) => {
                          console.log("post: ", data);
                          if (data.success) {
            ...
            onRowDelete: oldData =>
                new Promise((resolve, reject) => {
                    setTimeout(() => {
                      axios.delete('v1/channels/'+oldData.id
                        ).then(({data}) => {
                          console.log("post: ", data);
                          resolve();
                        }).catch(function (error) {
