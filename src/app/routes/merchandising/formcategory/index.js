...
class FormCategories extends React.Component {
  ...
  setActiveStatus = (id, value) => {
      axios.post('v1/form_category/status/'+id,
      {active : value}
      ).then(({data}) => {
  ...
  render() {
    return (
      ...
      <div className="app-wrapper">
        <ContainerHeader match={this.props.match} title="Manage Form Categories"/>
                ...
                return (
                  ...
                  control={
                    <Switch
                      ...
                      onChange={(e) => this.setActiveStatus(rowData.id, e.target.checked)}
                      color="primary"
                    />
                  }
                  label="Active"
          ...
          editable={{
            ...
            onRowUpdate: (newData, oldData) =>
                new Promise((resolve, reject) => {
                  console.log(newData)
                    setTimeout(() => {
                      axios.patch('v1/form_categories/'+newData.id,
                        newData
                        ).then(({data}) => {
                          console.log("post: ", data);
                          if (data.success) {
            ...
            onRowDelete: oldData =>
                new Promise((resolve, reject) => {
                    setTimeout(() => {
                      axios.delete('v1/teams/'+oldData.id
                        ).then(({data}) => {
                          console.log("post: ", data);
                          resolve();
                        }).catch(function (error) {
