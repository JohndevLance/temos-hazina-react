          ...
          columns={[
            ...
            {  
              ...
              render: (rowData) => {
                ...
                const then = moment(rowData.loctimestamp)
                const diff = now.diff(then)
                let duration = moment.utc(moment(now,"DD/MM/YYYY HH:mm:ss").diff(moment(then,"DD/MM/YYYY HH:mm:ss")))
                console.log(duration)
                let duration_formatted = duration.format("D [days], H [hours and] m [min]")
                let days = moment.duration(diff).days()
                if(days>7){
                  return <span className="badge badge-danger">{days} day(s) ago</span>
                }
                return <span className="badge badge-primary">{days} day(s) ago</span>
              } 
            },
          ]}
          detailPanel={[
