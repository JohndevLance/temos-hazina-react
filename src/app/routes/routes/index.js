...
import { getStockists } from 'actions/routes';
  ...
  render() {
    return (
      ...
      <div className="app-wrapper">
        <ContainerHeader match={this.props.match} title="Manage Routes"/>
        ...
        <MaterialTable
          ...
          columns={[
            ...
            {
              title: 'Stockist', field: 'stockist_id',
              render: (rowData) => {
                return rowData.stockist ? rowData.stockist.name : ''
              },
              editComponent: props => (
                <FormControl className="w-100 mb-2">
                  <InputLabel htmlFor="age-simple">Stockist</InputLabel>
                  <Select
                    value={props.value}
                    onChange={e => props.onChange(e.target.value)}
                    input={<Input id="stockist_id"/>}
                  >
                    <MenuItem value="">
                      <em>None</em>
                    </MenuItem>
                    {this.props.stockists_data.map((stockist) => {
                      return (
                        <MenuItem value={stockist.id}>{stockist.name}</MenuItem>
                      )
                    })}
          ...
          editable={{
            ...
            onRowUpdate: (newData, oldData) =>
                new Promise((resolve, reject) => {
                  console.log(newData)
                    setTimeout(() => {
                      axios.patch('v1/routes/'+newData.id,
                        newData
                        ).then(({data}) => {
                          console.log("post: ", data);
                          if (data.success) {
            ...
            onRowDelete: oldData =>
                new Promise((resolve, reject) => {
                    setTimeout(() => {
                      axios.delete('v1/routes/'+oldData.id
                        ).then(({data}) => {
                          console.log("post: ", data);
                          resolve();
                        }).catch(function (error) {
...
const mapDispatchToProps = dispatch => {
  return {
    getStockists: () => dispatch(getStockists())
  }
}
const mapStateToProps = ({ routes }) => {
  const { stockists_data } = routes;
  return { stockists_data }
};
