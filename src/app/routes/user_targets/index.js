  ...
  render() {
    return (
      ...
      <div className="app-wrapper">
        <ContainerHeader match={this.props.match} title="Manage User Targets"/>
        ...
        <MaterialTable
          ...
          columns={[
            ...
            { title: 'Strike Rate', field: 'strike_rate' },
            { title: 'Coverage', field: 'coverage' },
            { title: 'Lppc', field: 'lppc' },
          ]}
          data={query =>
            new Promise((resolve, reject) => {
              let url = 'v1/user_targets?'
                url += '&page=' + (query.page + 1)
                axios.get(url,
                ).then(({data}) => {
                  console.log("fetchUserTargets: ", data);
                  if (data.success) {
          ...
          editable={{
            ...
            onRowUpdate: (newData, oldData) =>
                new Promise((resolve, reject) => {
                  ...
                  delete newData.create_by
                  console.log(JSON.stringify(newData))
                    setTimeout(() => {
                      axios.put('v1/user_targets/'+newData.id,
                        newData
                        ).then(({data}) => {
            ...
            onRowDelete: oldData =>
                new Promise((resolve, reject) => {
                    setTimeout(() => {
                      axios.delete('v1/user_targets/'+oldData.id
                        ).then(({data}) => {
                          console.log("post: ", data);
                          resolve();
                        }).catch(function (error) {
