  ...
  render() {
    return (
      ...
      <div className="app-wrapper">
        <ContainerHeader match={this.props.match} title="Manage Distributors"/>
        ...
        <MaterialTable
          ...
          data={query =>
            new Promise((resolve, reject) => {
              let url = 'v1/distributors?'
                url += '&page=' + (query.page + 1)
                axios.get(url,
                ).then(({data}) => {
                  console.log("fetchdistributors: ", data);
                  if (data.success) {
          ...
          editable={{
            ...
            onRowUpdate: (newData, oldData) =>
                new Promise((resolve, reject) => {
                  console.log(newData)
                    setTimeout(() => {
                      axios.patch('v1/distributors/'+newData.id,
                        JSON.stringify(newData)
                        ).then(({data}) => {
                          console.log("post: ", data);
                          if (data.success) {
            ...
            onRowDelete: oldData =>
                new Promise((resolve, reject) => {
                    setTimeout(() => {
                      axios.delete('v1/distributors/'+oldData.id
                        ).then(({data}) => {
                          console.log("post: ", data);
                          resolve();
                        }).catch(function (error) {
