  ...
  render() {
    return (
      ...
      <div className="app-wrapper">
        <ContainerHeader match={this.props.match} title="Manage Types"/>
                ...
                axios.get(url,
                  ...
                  if(error.response){
                    ...
                    if(error.response.status==404){
                      this.notify('error', "404 Not Found")
                    }
                  }
                });
            })
          ...
          editable={{
            ...
            onRowUpdate: (newData, oldData) =>
                new Promise((resolve, reject) => {
                  console.log(newData)
                    setTimeout(() => {
                      axios.patch('v1/types/'+newData.id,
                        newData
                        ).then(({data}) => {
                          console.log("post: ", data);
                          if (data.success) {
            ...
            onRowDelete: oldData =>
                new Promise((resolve, reject) => {
                    setTimeout(() => {
                      axios.delete('v1/types/'+oldData.id
                        ).then(({data}) => {
                          console.log("post: ", data);
                          resolve();
                        }).catch( (error) => {
